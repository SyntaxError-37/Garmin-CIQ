...
module Bars {
    ...
    function drawProgressBarArc(dc as Dc,
        ...
        var _clampedPercent = percentComp >= 1.0 ? 0.9999 : percentComp;

        //calculates Arclenght, adjust values when crossing 3 o´clock position 
        var _arclength = arcEnd - arcStart;     
        if (direction.equals(Graphics.ARC_COUNTER_CLOCKWISE) && arcStart > arcEnd) {_arclength += 360;} else if (direction.equals(Graphics.ARC_CLOCKWISE) && arcStart < arcEnd) {_arclength -= 360;}      

        //calculates bar start and end
        var _barLength = Math.floor(_arclength * _clampedPercent);
        var _barStart = arcStart;
        var _barEnd = Math.floor(arcStart + _barLength);
        ...
        if (showPercent) {
            ...
            var _percentNumber = _clampedPercent * 100;
            var _percentDisp = _percentNumber.format("%.1f") + "%";
            var _arcDisp = arcStart + _barLength;
            _arcDisp = _arcDisp>360 ? _arcDisp - 360 : _arcDisp;
            _arcDisp = _arcDisp < 0 ? _arcDisp + 360 : _arcDisp;

            //sets text direction to the display percent value correctly 
            var _textdirection = _arcDisp < 180 ? Graphics.RADIAL_TEXT_DIRECTION_CLOCKWISE : Graphics.RADIAL_TEXT_DIRECTION_COUNTER_CLOCKWISE;           
            var _textradius = _textdirection.equals(Graphics.RADIAL_TEXT_DIRECTION_CLOCKWISE) ? r - padding - height + _borderwidth + 1 : r - padding - _borderwidth - 1;       
            var _textJustify = _clampedPercent <= 0.66 ? Graphics.TEXT_JUSTIFY_LEFT : Graphics.TEXT_JUSTIFY_RIGHT;
            if ((_arcDisp > 180 && direction.equals(Graphics.ARC_CLOCKWISE)) || (_arcDisp < 180 && direction.equals(Graphics.ARC_COUNTER_CLOCKWISE)))
                 { _textJustify = _clampedPercent <= 0.66 ? Graphics.TEXT_JUSTIFY_RIGHT : Graphics.TEXT_JUSTIFY_LEFT; }
    ...
    function drawProgressBarArcRounded(
        dc as Dc,
        xCenter as Number,
        yCenter as Number,
        r as Number,
        ...
        var _borderwidth = 1;
        var _clampedPercent = percentComp >= 1.0 ? 0.9999 : percentComp;
        var _rBarMid = r - height * 0.5 - padding;
        ...
        //calculates bar start and end
        var _barLength = Math.floor(_arclength * _clampedPercent);
        var _barStart = filling? arcStart : arcStart + _barLength;
        ...
        var _arcAdjust = Math.asin((height * 0.5) / (_rBarMid));
        var x1 = xCenter + Math.cos(Math.toRadians(arcStart) + _add * _arcAdjust) * (_rBarMid);
        var y1 = yCenter - Math.sin(Math.toRadians(arcStart) + _add * _arcAdjust) * (_rBarMid);

        //calculates arc start/end points, outer angle is startangle
        var _degStart = arcStart + _add * Math.toDegrees(_arcAdjust);
        var _barEndDirection = direction.equals(Graphics.ARC_CLOCKWISE) ? Graphics.ARC_COUNTER_CLOCKWISE : Graphics.ARC_CLOCKWISE;
        dc.drawArc(x1, y1, height * 0.5, _barEndDirection, _degStart, _degStart + 180);
        ...
        dc.drawArc(x1, y1, height * 0.5 + _thick * 0.5 + 2, _barEndDirection, _degStart, _degStart + 180);        

        //barend
        dc.setColor(_borderColor, Graphics.COLOR_TRANSPARENT);
        dc.setPenWidth(_borderwidth);
        x1 = xCenter + Math.cos(Math.toRadians(arcEnd) - _add * _arcAdjust) * (_rBarMid);
        y1 = yCenter - Math.sin(Math.toRadians(arcEnd) - _add * _arcAdjust) * (_rBarMid);
        _degStart = arcEnd - _add * Math.toDegrees(_arcAdjust);
        dc.drawArc(x1, y1, height * 0.5, direction, _degStart, _degStart + 180);
        dc.setColor(backgroundColor, Graphics.COLOR_TRANSPARENT);
        dc.setPenWidth(_thick+2);
        dc.drawArc(x1, y1, height * 0.5 + _thick * 0.5 + 2, direction, _degStart, _degStart + 180);
    }
}